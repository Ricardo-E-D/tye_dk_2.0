		...
		private void drawMap()
		{
			...
			if(objDr.Read())
			{
				...
				else
				{
                  ...
                  if (currLanguage == 4)
                  {
							strSql_lb = "SELECT users.id,CONCAT(users.zipcode, ' ', users.city, ', ', user_optician.name) AS optname FROM users INNER JOIN user_optician ON users.id = user_optician.userid WHERE regionid = " + Convert.ToInt32(strRegionId) + " AND isactive = 1 ORDER BY zipcode";
                  }
                  else
                  {
                      strSql_lb = "SELECT users.id,CONCAT(user_optician.name,', ',users.city) AS optname FROM users INNER JOIN user_optician ON users.id = user_optician.userid WHERE regionid = " + Convert.ToInt32(strRegionId) + " AND isactive = 1 ORDER BY users.city, user_optician.name";
                  }
