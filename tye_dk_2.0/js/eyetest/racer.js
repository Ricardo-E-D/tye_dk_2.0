...
var points = 10000;
var onEdge = 0, intCompensateX = 0, intCompensateY = 0;
...
function SetVars() {
	...
	onEdge = 0;
	score = 0;
	objCar = document.getElementById('car');
}

function LoadCars() {
	for (var i = 0; i <= 360; i += 10) {
		var img = new Image();
		img.src = "/img/eyetest/racer/car/" + i + ".png";
		cars.push(img);
	}
	$('#car').attr('src', cars[0].src);
}

//Vector Functions
function GenerateVectorLines() {
	for (var i = 0; i < map.length - 2; i++) {
		var x = map[i] + intCompensateX;
		var y = map[++i] + intCompensateY;
		var x2 = map[++i] + intCompensateX;
		var y2 = map[++i] + intCompensateY;
...
function CheckPositionOnTrack(screenCoord) {
	if (start == 1) {
		...
		$('#maps').addClass('hideCursor');
	}

	if (lineNr == (map1Arr.length - 1)) { // level completed
		dtEnd = new DateTime();
		var totalSeconds = parseInt(dtEnd.subtractDate(dtStart).totalSeconds(), 10);
		score = points - (totalSeconds * 25) - (edgeCrashes * 100);
		end = 1;
		...
		eyeTestScreen.stop(); // log
		alert(eyeTestScreen.messages.congratulations
					.replace('[seconds]', totalSeconds)
					.replace('[errors]', edgeCrashes)
					.replace('[score]', score)
			);
		$('#maps').removeClass('hideCursor');
		DetectScreenSize();
	}
	...
	if (CompareSigns(pVector, v.vector) == true) {
		...
		var ang = GetAngle(v.vector, new Vector(10, 0)),
			//carPosOffsetX = 10, carPosOffsetY = 25; // used to be 12 for both
			carPosOffsetX = 12, carPosOffsetY = 12; // used to be 12 for both
		
		if (dist <= roadWidth) {
			objCar.src = GetCarByAngle(v.vector, ang);
			$(objCar).css({
				left: screenCoord.x - carPosOffsetX + addWidth,
				top: screenCoord.y - carPosOffsetY + addHeight
			});
			start += 1;
			onEdge = 0;
...
function processCoordinate(e) {
    ...
    // touches devices
	if (typeof undefined != typeof e.originalEvent) {
	    if (typeof undefined != typeof e.originalEvent.touches) { 
	        touchdevice = true;
	...
	if (end != 1) {
		var c = new Coord(posx - addWidth, posy - addHeight - 25)
		CheckPositionOnTrack(c);
	} else {
		if (start >= 1 && totalCrashes == 1) {
			objCar.src = "/img/eyetest/racer/car/explode.png";
			start = 0;
			totalCrashes = 0;
			edgeCrashes = 0;

			eyeTestScreen.stop(); // log
			alert(eyeTestScreen.messages.offTrack);
...
function setMap() {
	map = new Array();
	var number = document.getElementById("mapNum"), level = 1;
	if (number.value == "map1a") {
		intCompensateX = (474-491), intCompensateY = (356-362);
		...
		level = 2;
		intCompensateX = (469 - 491), intCompensateY = (353 - 362);
		...
		level = 3;
		intCompensateX = (444 - 491), intCompensateY = (361 - 362);
		...
		level = 4;
		intCompensateX = (439 - 491), intCompensateY = (359 - 362);
		map = new Array(84, 47, 126, 79, 154, 92, 202, 94, 287, 69, 350, 41, 389, 37, 418, 56, 424, 81, 413, 119, 385, 159, 352, 191, 319, 209, 280, 220, 253, 231, 234, 261, 226, 303, 232, 335, 252, 340, 287, 332, 338, 296, 358, 268, 346, 227, 330, 190, 314, 170, 289, 156, 251, 150, 226, 152, 188, 167, 164, 189, 156, 218, 157, 266, 154, 290, 136, 306, 104, 309, 78, 299, 67, 275, 69, 242, 85, 202, 123, 142, 148, 107, 171, 69, 193, 20);
	} else if (number.value == "map3a") {
		level = 5;
		intCompensateX = (437 - 491), intCompensateY = (342 - 362);
		...
		level = 6;
		intCompensateX = (432 - 491), intCompensateY = (339 - 362);
		...
		level = 7;
		intCompensateX = (436 - 491), intCompensateY = (337 - 362);
		...
		level = 8;
		intCompensateX = (433 - 491), intCompensateY = (333 - 362);
		map = new Array(89, 56, 115, 84, 143, 110, 177, 138, 217, 162, 257, 180, 311, 194, 349, 206, 397, 226, 415, 252, 409, 274, 385, 300, 359, 308, 333, 302, 315, 270, 313, 224, 337, 186, 377, 130, 385, 86, 371, 60, 329, 48, 255, 48, 199, 58, 151, 78, 121, 98, 97, 134, 81, 164, 75, 210, 75, 250, 83, 282, 95, 308, 109, 314, 141, 300, 179, 258, 211, 212, 247, 154, 255, 142, 279, 98);
	} else if (number.value == "map5a") {
		level = 9;
		intCompensateX = (436 - 491), intCompensateY = (348 - 362);
		...
		level = 10;
		intCompensateX = (434 - 491), intCompensateY = (344 - 362);
	...
	$('#maps').attr('src', '/img/eyetest/racer/map/' + number.value + '.png');
}

function DetectScreenSize() {
	SetVars();
	var w = $(window);
	browseWidth = w.innerWidth();
	browseHeight = w.innerHeight();
	addWidth = parseInt((browseWidth / 2), 10) - 245; // 245
	addHeight = parseInt((browseHeight / 2), 10) - 194; //181
	...
	$('#car').css(
		{
			top: addHeight + map1Arr[0].coord1.y - parseInt($('#car').height() / 2, 10),
			left: addWidth + map1Arr[0].coord1.x - parseInt($('#car').width() / 2, 10)
		}
	);
	$(window).on('touchstart', function (event) {
		var t = $(event.target);
		if(t.parents('.eyeTestScreenTopMenu').length < 1 && t.parents('.closebuttons').length < 1) {
			event.preventDefault();
		}
	});

	$(window).on('mousemove touchmove', function (event) {
		processCoordinate(event);
	});
